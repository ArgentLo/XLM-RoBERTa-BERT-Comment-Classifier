...
from model import XLMRobertaBase
...
def run(model_path):
    ...
    device = "cuda"
    model  = XLMRobertaBase().to(device)
    # For multiple GPUs
    ...
    test_dataset = dataset.XLMRobertaDatasetTest(comment_text=df_en.content_en.values)
    ...
    test_dataset = dataset.XLMRobertaDatasetTest(comment_text=df_en.translated.values)
    print(f"test size: {len(test_dataset)}")
    test_data_loader = torch.utils.data.DataLoader(
        test_dataset,
        batch_size=config.TEST_BATCH_SIZE,
...
if __name__ == "__main__":
    ...
    trained_model = "roberta_base_4.bin"
